...
void examine(Pet& p)
{
    const type_info& type = typeid(p);
    cout << "animal is of type " << type.name() << "\n";
}
...
int main() {
    ...
    Pet* petpointer;
    Bird* birdTest = static_cast<Bird*>(petpointer);
    Fish* fishTest = static_cast<Fish*>(petpointer);
    //int* intTest = static_cast<int*>(petpointer);  //Throws a compiler error
    
    const Bird* bird = new Bird("bird");
    Bird* birdNonConst = const_cast<Bird*>(bird);
    //Fish* fishNonConst = const_cast<Fish*>(bird); // throws a compiler error
